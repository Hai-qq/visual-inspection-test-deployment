...
- 当前操作员工作台以检测图为主体，左右辅助栏宽 210/240 px，底部六条规则按两列三行显示，运行日志默认折叠。未导入时显示网格空状态；导入或应用用户图源后显示其预览内容。默认内置项目为“FAN-A01 视觉检测项目”，生产型号为 `FAN-A01`，演示包预装 `fan.onnx` 与 `IMG_1533.JPG`；唯一“风扇检测”测试步以 AND 组合 6 条标签规则。操作台顶栏提供“导入测试序列”和管理员“测试序列设置”，运行端继续通过经过验证的 v1 兼容 Runner 执行受支持的 ONNX Detection。
...
- 自定义函数类型、名称、文件路径和说明随 Draft、Sequence 导出及操作台兼容配置保存，重新打开时恢复。当前只保存函数元数据，不执行函数；测试步延时仍生效，页面直接说明这一边界。
- 操作台成功导入后保存项目，启动时恢复最近保存的项目；用户导入或应用的配置不被内置示例刷新覆盖。`--verify-portable-sequence` 保持只读。视频文件夹使用独立类型，当前操作台拒绝新旧视频配置，不再按图片目录运行。
