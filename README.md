...
仓库包含操作员工作台和管理员测试序列设置界面的完整源码，功能以当前检出版本为准。无需复制原开发电脑上的 `bin`、`obj` 或 `artifacts`。
...
普通启动及 `--operator-preview` 均进入“未加载测试序列”状态，不生成或恢复示例项目。导入 Sequence 或由管理员完成空白设置并应用后，才显示型号、测试步与规则。确定性图片与 `detections.json` 只用于显式验收命令，**不是实际模型推理**。真实 ONNX 推理、Sequence 导入导出及结果保存的代码均包含在仓库中；真实检测需提供符合下述契约的 ONNX 模型与本地图片。模型、现场图片和原电脑的运行配置不随源码公开上传。内置 Fan 模型的自包含演示包需另行提供脚本要求的 Fan 模型与图片才能构建，普通源码构建不依赖这些文件。
...
- 当前操作员工作台以检测图为主体，左右辅助栏宽 210/240 px，底部六条规则按两列三行显示，运行日志默认折叠。未导入时显示网格空状态；导入或应用用户图源后显示其预览内容。默认不加载任何型号或测试步；显式 Fan 验收夹具使用 `FAN-A01`，一个“风扇检测”测试步以 AND 组合 6 条标签规则。操作台顶栏提供“导入测试序列”和管理员“测试序列设置”，运行端继续通过经过验证的 v1 兼容 Runner 执行受支持的 ONNX Detection。
...
- 新增测试步后通过弹窗选择与检测类型兼容的模型。测试步列表与检测标签区独立滚动，默认设置窗口完整显示六条标签；整张图规则使用紧凑表单，ROI 编辑区随窗口伸缩，最小窗口下保存按钮仍完整可见。用户可见“检测子项”术语统一为“检测标签”。所有测试步统一参与序列执行和产品总判定，界面不再显示容易误解的“是否必选”；底层兼容字段仍保留。置信度字段移到规则区底部，默认 `0.5` 且可留空；其他必填字段继续使用红色 `*`。
- 每个检测标签独立保存 Full Image/单默认 ROI、坐标和参照图尺寸，重新编辑一个标签保留其他规则及稳定标识。新增或重画 ROI 须导入标注底图；已有有效 ROI 可在无底图时回填并原样保存。预览使用等比例 `Uniform` 显示，灰色留白不参与框选。标注底图只用于设置，不写入 sequence，也不作为生产运行输入。
- 缺失数量规则分别配置“应有总数”和比较阈值，计算 `max(应有总数 - 识别数量, 0)`。已有配置载入后完整回填全部规则及实际摘要；不会将缺失判定阈值误当作应有总数。
- 自定义函数类型、名称、文件路径和说明随 Draft、Sequence 导出及操作台兼容配置保存，重新打开时恢复。导入时未配置函数的测试步继续保持为空，重新导出不会补入示例函数。当前只保存函数元数据，不执行函数；测试步延时仍生效，页面直接说明这一边界。
- 操作台成功导入后保存项目，但每次重新启动都等待显式导入，不自动恢复历史项目；历史文件保留，启动不读取或覆盖它们。`--verify-portable-sequence` 保持只读。视频文件夹使用独立类型，当前操作台拒绝新旧视频配置，不再按图片目录运行。
- 运行图像标识显示实际图像像素尺寸及 ONNX/验收数据状态；长状态消息分列省略并可悬停查看。检测标签避让已有标签，先画检测框再画文字，避免边线遮字。
- 向导内的说明式 ToolTip 统一使用可见深色样式、约 150 ms 显示延迟、20 秒显示时长；信息图标扩展为至少 22 × 22 的透明命中区，禁用控件上的说明也可显示。
- 界面已按默认尺寸及 1120 × 720 最小窗口复查：五步导航自动留足上下边框空间，模型页六个常用英文标签完整显示，函数表单与姿态动作参数不再被内容区裁切；操作台数量/合格率图随可用高度缩放。布局复核命令 `--ui-layout-review` 生成本地 WPF 渲染图，详情见 [验收说明](./docs/验收说明.md)；当前复查记录见 `design-qa.md` Pass 23。
- “应用与导出”默认只展示基础检查、型号、图源、模型数及测试步数；问题可直接返回对应步骤修改，详细配置默认折叠，固定运行说明可悬停或点击按钮打开 ToolTip。导出文件期间暂时禁用编辑、导航、应用和关闭；失败后恢复操作并清除旧成功提示。已配置函数继续提示仅保存、不执行。基础检查不代替模型文件、兼容性和运行就绪校验；应用与导出仍为独立操作。
- 目标检测、图像分割与姿态测试步仍只显示“基本信息、自定义函数”；图像分割、Pose/Temporal、PT、视频、相机/PLC/IO 若无对应 C# 适配器必须保持运行门禁。自定义函数为保留元数据，不会随视觉测试执行；界面可配置不代表函数执行已接入。
...
默认产物位于 `artifacts\frontend-demo-v0.2-YYYYMMDD-win-x64\VisualInspection.FrontendDemo.exe`。使用当前源码重新构建的演示 EXE 双击后先显示登录页；登录后进入未加载状态，需显式导入 Sequence 与模型后才能检测。Fan 资源仍用于显式验收命令和代表性 Sequence 导出；“测试序列设置”可直接应用当前配置，也可另选位置导出 Sequence 与模型。该演示 EXE 不要求目标机器另行安装 .NET 8 Desktop Runtime；显式验收或代表性 Sequence 导出时解包内置模型和图片可能需要数秒。此结果只证明指定模型与指定图片的接入链，不代表通用精度、现场节拍或生产能力；视频读取、相机/PLC/IO、分割/姿态推理、自定义函数及 V2 生产运行能力仍未扩展。
...
- 操作工工作台以中部实时检测图为主体：左侧窄栏提供序列和执行按钮，右侧提供会话统计与单一图表，底部 174 px 显示当前项和两列规则明细；日志按需展开；
