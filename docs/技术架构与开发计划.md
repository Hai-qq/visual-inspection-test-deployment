...
- `v1.0.0` 标签中的 Admin Test Sequence Settings 提供七个可保存的功能分区；当前 V2 分支新增独立八步 WPF 向导原型、前端项目多模型集合、有序检测项加减排序、动态模型选择、必选状态和目标/姿态类型联动，只承载前端状态，不依赖 Core 编辑器、真实模型导入或项目存储，待界面评审后再迁移旧版功能；
...
`ProjectConfigurationValidator` 检查引用完整性、模型绑定、标签、ROI、数量参数、置信度、Delay、Pose 时间、图源互斥和发布字段。项目存储为 schema v1；不支持的 schema 会阻止加载。当前没有跨 schema 自动迁移和发布快照回退。
...
`--acceptance-smoke` 使用与 UI 相同的 Folder、清单适配器和 Runner，但构造隔离的标准示例项目，避免管理员保存的 ROI 或图源影响自动门禁，也不会覆盖普通项目配置。退出码 `0/2/3` 分别代表 Pass/Fail/Error，并生成结构化回执。发布脚本还执行 `--ui-construction-smoke`，实际显示、布局并渲染 `LoginWindow`、`MainWindow`、`InputSourceSettingsWindow`、经典 `TestSequenceSettingsWindow` 和 `TestSequenceWizardV2Window`，在隔离配置草稿中执行一次普通检测项新增/删除，并检查 V2 的跳步不补绿、必填失效退绿、未完成步骤拦截、前后导航、USB 图源卡绿色选中、多模型库加减与类型切换、检测项动态模型绑定、检测项加减、目标/姿态类型联动、ROI 框选/坐标回填、目标与姿态判定摘要实时联动、数量范围校验、ToolTip 存在性和最终确认状态。`--v2-wizard-preview` 可独立打开前端评审窗口，`--v2-wizard-models-preview`、`--v2-wizard-roi-preview` 和 `--v2-wizard-rule-preview` 分别直接打开多模型库、ROI 框选与判定条件步骤；`--v2-wizard-snapshot`、`--v2-wizard-items-snapshot`、`--v2-wizard-pose-snapshot`、`--v2-wizard-source-snapshot`、`--v2-wizard-models-snapshot`、`--v2-wizard-roi-snapshot` 和 `--v2-wizard-rule-snapshot` 分别生成首屏、检测项编排、姿态内容、USB 图源选中、多模型库、ROI 框选和判定条件视觉快照。这些入口用于捕获编译阶段发现不了的 XAML 属性转换、绑定、资源加载和导航异常；随后执行正常启动生命周期冒烟，确认异步初始化期间应用不会在登录窗口出现前提前退出。
...
| M8 | Test Sequence Setting V2 顺序向导 | 八步前端、基于必填校验与明确确认的步骤完成状态、顶部跳转、前后导航、项目多模型库加减与独立设置、检测项动态模型选择、有序检测项加减排序、必选状态、目标/姿态类型联动、ROI 鼠标框选与参考坐标回填、目标/姿态判定摘要实时联动、数量范围输入校验、红星、ToolTip、汇总确认和视觉快照完成；真实模型导入、真实图源背景、配置绑定、保存、Runner 校验与发布待界面评审通过后接入 |
...
- V2 八步向导的步骤命名、多模型库布局、有序检测项操作、必选状态、类型联动、每页字段密度和最终汇总需由用户确认；确认前不迁移旧版保存逻辑。
