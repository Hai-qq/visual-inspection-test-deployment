...
| --- | --- |
| 文档版本 | v0.8 |
| 当前里程碑 | v0.6.0 ONNX YOLO 端到端推理验收版 |
| 最近核验日期 | 2026-08-14 |
| 强制开发语言 | C# |
...
- Admin Test Sequence Settings 已提供七个配置分区，可新增/删除普通检测项，并编辑模型/标签、目标主绑定、普通规则、ROI、姿态步骤和延时后统一校验保存；新增普通检测项由 Core 编辑器生成可校验的默认规则，删除后统一压缩测试项顺序；
...
| 项目 | 当前职责 | 依赖边界 |
| --- | --- | --- |
| `VisualInspection.App` | WPF 工作台、Admin 图源页、测试序列设置页、ViewModel、内置验收数据、启动/冒烟编排 | 依赖 Core、Infrastructure |
...
`--acceptance-smoke` 使用与 UI 相同的 Folder、清单适配器和 Runner，但构造隔离的标准示例项目，避免管理员保存的 ROI 或图源影响自动门禁，也不会覆盖普通项目配置。退出码 `0/2/3` 分别代表 Pass/Fail/Error，并生成结构化回执。发布脚本还执行 `--ui-construction-smoke`，实际显示、布局并渲染 `LoginWindow`、`MainWindow`、`InputSourceSettingsWindow` 和 `TestSequenceSettingsWindow`，并在隔离配置草稿中执行一次普通检测项新增/删除，用于捕获编译阶段发现不了的 XAML 属性转换、绑定、资源加载和编辑事件异常；随后执行正常启动生命周期冒烟，确认异步初始化期间应用不会在登录窗口出现前提前退出。
