...
- `v1.0.0` 标签中的 Admin Test Sequence Settings 提供七个可保存的功能分区；当前 V2 分支新增独立九步 WPF 向导原型和预览/快照入口，只承载前端导航状态，不依赖 Core 编辑器或项目存储，待界面评审后再迁移旧版功能；
...
`--acceptance-smoke` 使用与 UI 相同的 Folder、清单适配器和 Runner，但构造隔离的标准示例项目，避免管理员保存的 ROI 或图源影响自动门禁，也不会覆盖普通项目配置。退出码 `0/2/3` 分别代表 Pass/Fail/Error，并生成结构化回执。发布脚本还执行 `--ui-construction-smoke`，实际显示、布局并渲染 `LoginWindow`、`MainWindow`、`InputSourceSettingsWindow`、经典 `TestSequenceSettingsWindow` 和 `TestSequenceWizardV2Window`，在隔离配置草稿中执行一次普通检测项新增/删除，并检查 V2 的前后导航、姿态跳过和最终确认状态。`--v2-wizard-preview` 可独立打开前端评审窗口，`--v2-wizard-snapshot` 将首屏快照写入 `%LocalAppData%\VisualInspectionTestDeployment\v2-wizard-preview.png`。这些入口用于捕获编译阶段发现不了的 XAML 属性转换、绑定、资源加载和导航异常；随后执行正常启动生命周期冒烟，确认异步初始化期间应用不会在登录窗口出现前提前退出。
...
| M8 | Test Sequence Setting V2 顺序向导 | 九步前端、顶部跳转、前后导航、可跳过姿态、汇总确认和视觉快照完成；配置绑定、保存、校验与发布待界面评审通过后接入 |
