...
当前兼容操作台仍保存 schema v1 项目到 `projects`，不是切换上述原生 V2 Active Pointer。成功导入/应用的项目标记 `isUserConfigured=true`，启动按最近保存时间选择并跳过内置示例重置；只读便携校验不会写项目。旧版客户端不认识新增视频枚举，需使用本次更新的客户端；缺少 `customFunction` 的旧配置仍可载入。
