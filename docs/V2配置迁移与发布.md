...
校验问题继续使用稳定 `Code + Path + Message + Severity`，内部 Publish 命令在存在 Error 时保持禁用。当前前端确认稿的最终页不显示校验、Publish 或生命周期状态。
...
| 现场连接 | 不猜测；`DeploymentBindings=[]`，必须现场补齐后才能 Runtime Validated |

迁移使用确定性 Guid 派生 Runtime Profile 和 Invocation ID，重复加载相同 v1 文件得到相同身份。迁移不写入虚假的 PLC、相机或 Line 地址，也不会把 `detections.json` 升格为生产模型。
