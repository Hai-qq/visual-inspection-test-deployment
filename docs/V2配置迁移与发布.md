...
2026-09-07 配置回读修复：规则的 Scope/ROI、ExpectedCount 与 Threshold 分别保存并回填；标签编辑保留规则和模型绑定 ID。Test Step 的可选 `customFunction` 包含 `kind`（PythonFile/BuiltIn/Reserved）、`name`、`filePath`、`description`，经过 V1 兼容转换和迁移仍保留，仅作为元数据，不执行函数。视频文件夹使用 `InputSourceKind.VideoFolder`；操作台同时拒绝旧版以 Folder +“视频文件夹”名称表达的配置。

当前兼容操作台仍保存 schema v1 项目到 `projects`，不是切换上述原生 V2 Active Pointer。成功导入/应用的项目标记 `isUserConfigured=true`，启动不自动选择历史项目，也不创建内置示例；保留项目文件，等待用户显式导入；只读便携校验不会写项目。旧版客户端不认识新增视频枚举，需使用本次更新的客户端；缺少 `customFunction` 的旧配置仍可载入。
