...
> 状态（2026-08-27）：这是 2026-08-18 冻结的从零开发历史基线，不是当前分支的现役规格。它未包含 schema v2 生产基础、8.27 SE 会议冻结的图源区分序列号流程、sequence 驱动的标签/判定逻辑/实测/Result 展示、单 ROI 背景图、模型/标签编辑、可移植 sequence 交付及生产 TXT/图片落盘。继续维护当前仓库时必须以 [8.27 SE 会议变更基线](./8.27SE会议变更基线.md)、[软件需求规格说明](./软件需求规格说明.md)、[前端 UI 概要设计](./前端UI概要设计.md)、[技术架构与开发计划](./技术架构与开发计划.md) 和 [验收说明](./验收说明.md) 为准；如要把本提示词重新作为新项目输入，应先单独立项升级，不能直接复制后声称与当前实现一致。
>
